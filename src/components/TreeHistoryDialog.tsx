...
import { TreeMarker, STATUS_LABELS, CONDITION_LABELS } from '@/types';
...
export default function TreeHistoryDialog({ tree, onRollback, onClose }: Props) {
  const [history, setHistory] = useState<HistoryEntry[]>([]);
  const [loading, setLoading] = useState(true);
  const [rolling, setRolling] = useState<number | null>(null);

  useEffect(() => {
    fetch(`${API.TREES}?id=${tree.id}&action=history`)
      .then(r => r.json())
      .then(data => {
  ...
  const handleRollback = async (entry: HistoryEntry) => {
    setRolling(entry.id);
    const res = await fetch(`${API.TREES}?id=${tree.id}&action=rollback&history_id=${entry.id}`, {
      method: 'PUT',
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-gray-100">
          <div>
            ...
            <div className="text-xs text-[#6b7c6e]">{tree.name} — №{tree.number}</div>
          </div>
        ...
        <div className="overflow-y-auto flex-1 p-4 space-y-3">
          ...
          {history.map((entry, i) => {
            ...
            const next = i === 0 ? tree : history[i - 1].snapshot;
            return (
              <div key={entry.id} className="border border-gray-100 rounded-lg p-3">
                ...
                <div className="space-y-0.5">
                  ...
                  <DiffRow label="Порода" before={snap.species} after={next.species} />
                  <DiffRow label="Диаметр" before={`${snap.diameter} см`} after={`${next.diameter} см`} />
                  <DiffRow label="Высота" before={`${snap.height} м`} after={`${next.height} м`} />
                  <DiffRow label="Кол-во" before={snap.count} after={next.count} />
                  <DiffRow label="Возраст" before={snap.age} after={next.age} />
                  ...
                  <DiffRow label="Жизнь" before={LIFE_STATUS_LABELS[snap.lifeStatus] ?? snap.lifeStatus} after={LIFE_STATUS_LABELS[next.lifeStatus] ?? next.lifeStatus} />
